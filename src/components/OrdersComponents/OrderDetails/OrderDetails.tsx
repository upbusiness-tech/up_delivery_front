    ...
    <>
      <style>{`
        ...
        @media print {
          ...
          #print-order { display: block; position: absolute; top: 0; left: 0; width: 100%; }
        }
      `}</style>
        ...
        <DialogContent dividers>
          <Grid container spacing={2}>
            ...
            <Grid size={{ xs: 12, md: 5 }}>
              <Typography variant="subtitle1" gutterBottom>Dados:</Typography>
              <Stack spacing={2}>
                <Paper elevation={0} sx={{ p: 1 }}>
                  ...
                  {isDelivery ? (
                    ...
                    <>
                      <Typography variant="subtitle2" gutterBottom>Modalidade</Typography>
                      <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
                        <StorefrontIcon fontSize="small" sx={{ color: "grey.500" }} />
                        <Typography variant="body2" color="text.secondary">Retirada no local</Typography>
                      </Stack>
                    </>
                  )}
                </Paper>
                <Divider sx={{ my: 1 }} />
                <Paper elevation={0} sx={{ p: 1 }}>
                  <Typography variant="subtitle2" gutterBottom>Pagamento</Typography>
      ...
      <Dialog open={openModalOrderCancel} onClose={handleCloseModalOrderCancel}>
        <DialogTitle>
          ...
          <IconButton aria-label="Fechar" onClick={handleCloseModalOrderCancel} sx={{ position: "absolute", right: 12, top: 12 }}>
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>
          <Grid size={{ xs: 12, md: 7 }}>
            <Typography variant="subtitle2" gutterBottom>Essa ação não poderá ser desfeita!</Typography>
          </Grid>
        </DialogContent>
        <DialogActions>
          <Stack direction="row" spacing={1}>
            <Button sx={{ textTransform: "none" }} disableElevation variant="contained" color="inherit" size="medium" onClick={() => handleCloseModalOrderCancel()}>
              Voltar
            </Button>
            <Button sx={{ textTransform: "none" }} variant="contained" color="error" size="medium" onClick={() => cancelOrder()}>
              Cancelar
            </Button>
          </Stack>
        </DialogActions>
