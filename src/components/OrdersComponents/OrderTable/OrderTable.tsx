...
export function OrderTable({ orders, loading, isDesktop, updateStatusOrder, deliveryFee, openOrderDetail }: Props) {
  ...
  const handleReceived = (e: React.MouseEvent, order: Order) => {
    e.stopPropagation();
    setReceivedOrders((prev) => prev.includes(order.id) ? prev : [...prev, order.id]);
                      ...
                      {isCancelled ? (
                        ...
                        <Stack direction="row" spacing={1} sx={{ justifyContent: "flex-end", alignItems: "center" }}>
                          <Button
                            ...
                            variant={receivedOrders.includes(order.id) ? "outlined" : "contained"}
                            size="small"
                            onClick={(e) => handleReceived(e, order)}
                            sx={{ minWidth: 95, height: 34, textTransform: "none", borderRadius: 2, fontWeight: 600, backgroundColor: receivedOrders.includes(order.id) ? "transparent" : "#25D366", color: receivedOrders.includes(order.id) ? "#64748B" : "#fff", borderColor: receivedOrders.includes(order.id) ? "#CBD5E1" : "#25D366", "&:hover": { backgroundColor: receivedOrders.includes(order.id) ? "#F8FAFC" : "#20BD5A", borderColor: receivedOrders.includes(order.id) ? "#94A3B8" : "#20BD5A" } }}
                          >
                            {receivedOrders.includes(order.id) ? "Recebido" : "Receber"}
                          </Button>

                          <Select value={order.status} size="small" onChange={(e) => updateStatusOrder(e.target.value, order.id)} onClick={(e) => e.stopPropagation()} sx={{ minWidth: 135, height: 34, color: "#fff", fontWeight: 700, fontSize: "0.78rem", borderRadius: 2, backgroundColor: STATUS_COLOR(order.status), "& .MuiOutlinedInput-notchedOutline": { border: "none" }, "&:hover .MuiOutlinedInput-notchedOutline": { border: "none" }, "& .MuiSelect-icon": { color: "#fff" } }}>
                            {Object.values(STATUS).filter((status) => status !== STATUS.CANCELADO).map((status) => (
          ...
          <Paper key={order.id} elevation={0} onClick={() => openOrderDetail(order)} sx={{ p: 2, cursor: "pointer", border: "1px solid #E5E7EB", borderRadius: 3, backgroundColor: isCancelled ? "#FFF7F7" : "#fff", transition: "all 0.15s ease", "&:hover": { backgroundColor: isCancelled ? "#FFF1F2" : "#F8FAFC", borderColor: "#CBD5E1" } }}>
            ...
            {isCancelled ? (
              ...
              <Stack direction="row" spacing={1}>
                ...
                <Select value={order.status} size="small" onChange={(e) => updateStatusOrder(e.target.value, order.id)} onClick={(e) => e.stopPropagation()} sx={{ flex: 1, height: 36, color: "#fff", fontWeight: 700, fontSize: "0.78rem", borderRadius: 2, backgroundColor: STATUS_COLOR(order.status), "& .MuiOutlinedInput-notchedOutline": { border: "none" }, "&:hover .MuiOutlinedInput-notchedOutline": { border: "none" }, "& .MuiSelect-icon": { color: "#fff" } }}>
                  {Object.values(STATUS).filter((status) => status !== STATUS.CANCELADO).map((status) => (
                    <MenuItem key={status} value={status}>{STATUS_LABEL[status]}</MenuItem>
                  ))}
                </Select>
